...
if pid == 0:
    ...
    OUT_STATUS = 0
    IN_STATUS = 1
    IN_RECOMMEND_FINISHED = 2
    OUT_RECOMMEND_FINISHED = 3
    while True:
        db.session.commit()
        UNRECOMMENDED_IN_STATUS = inorout.query.filter(inorout.inorout == IN_STATUS)
        UNRECOMMENDED_OUT_STATUS = inorout.query.filter(inorout.inorout == OUT_STATUS)
        if UNRECOMMENDED_IN_STATUS.first():
            come_time = UNRECOMMENDED_IN_STATUS.first().come_time
            ...
            if line_account is not None:
                ...
                time.sleep(3)
                inorout.query.filter(inorout.come_time == come_time).update({"inorout": IN_RECOMMEND_FINISHED})
                db.session.commit()
            else:
                time.sleep(10)
        elif UNRECOMMENDED_OUT_STATUS.first():
            come_time = UNRECOMMENDED_OUT_STATUS.first().come_time
            phone_number = inorout.query.filter(inorout.come_time == come_time).one().phone_number
            line_account = member.query.filter(member.phone_number == phone_number).one().line_account
            # make sure we have buyer's line_id
            if line_account is not None:
                ...
                time.sleep(1)
                # update status
                inorout.query.filter(inorout.come_time == come_time).update({"inorout": OUT_RECOMMEND_FINISHED})
                db.session.commit()
        else:
            time.sleep(1)
else:
    ...
    def handle_follow_event(event):
        try:
            ...
            linkMenuRequestHeader = {'Content-Type': 'image/png',
                                     'Authorization': f'Bearer {CHANNEL_ACCESS_TOKEN}'}  # 設定 headers
    ...
    def handle_post_message(event):
        ...
        if event.postback.data.find('[menu]') == 0:
            if event.postback.data[6:] == 'location':
                line_bot_api.reply_message(event.reply_token,
                                           LocationSendMessage(title='吃冰', address='320桃園市中壢區中大路300號',
                                                               latitude=24.967798, longitude=121.191802))
            elif event.postback.data[6:] == 'web':
                line_bot_api.reply_message(event.reply_token,
                                           TextSendMessage(text='https://35.194.195.218:8000/index/'))
            elif event.postback.data[6:] == 'customerService':
                line_bot_api.reply_message(event.reply_token, TextSendMessage(
                    text='Any comment is welcome.\nTel: 03-1231231\nE-mail: customerService@eatice.com.tw'))
        # surveyTemplate function
        elif event.postback.data.find('[surveyTemplate]') == 0:
            satisfaction = re.match('\[surveyTemplate\](\d),(\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)',
                                    event.postback.data).group(1)
            come_time = re.match('\[surveyTemplate\](\d),(\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)',
                                 event.postback.data).group(2)
            inorout.query.filter(inorout.come_time == come_time).update({'satisfaction': satisfaction})
            db.session.commit()
            ...
            neededcall = re.match('\[neededcall\](\d),(\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)', event.postback.data).group(
                1)
            come_time = re.match('\[neededcall\](\d),(\d\d\d\d-\d\d-\d\d\s\d\d:\d\d:\d\d)', event.postback.data).group(
                2)
            ...
            if int(neededcall) == 1:
                ...
                line_bot_api.reply_message(event.reply_token,
                                           TextSendMessage(text="好的，如果有任何意見，歡迎寫信到customer_service@eatice.com.tw"))
